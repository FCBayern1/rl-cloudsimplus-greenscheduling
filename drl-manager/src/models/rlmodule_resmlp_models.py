"""
ResMLP-based RLModules (vector-friendly) for Multi-DC PPO using RLlib New API Stack.

This keeps the same external behavior as `rlmodule_models.py`:
- Local agents: Discrete actions with action masking
- Global agent: Discrete or MultiDiscrete actions (MultiCategorical distribution)
"""
...
from src.networks.mlp_backbones import BackboneConfig, ResMLPBackbone

logger = logging.getLogger(__name__)


def _cfg_from_model_config(model_config: Dict[str, Any]) -> BackboneConfig:
    """Map RLModule model_config dict -> BackboneConfig with reasonable defaults."""
...
class ResMLPMaskedActionRLModule(TorchRLModule, InferenceOnlyAPI, ValueFunctionAPI):
    """Local-agent PPO RLModule with action masking, using ResMLP backbone."""
    ...
    def setup(self):
        ...
        if not isinstance(action_space, spaces.Discrete):
            raise ValueError(
                f"ResMLPMaskedActionRLModule requires Discrete action space, got {type(action_space)}"
            )
        ...
        self.backbone = ResMLPBackbone(obs_dim, cfg)
...
class ResMLPDictObsRLModule(TorchRLModule, InferenceOnlyAPI, ValueFunctionAPI):
    """Global-agent PPO RLModule without action masking, using ResMLP backbone."""
    ...
    def setup(self):
        ...
        if isinstance(action_space, spaces.Discrete):
            ...
            logger.warning(
                "[ResMLPDictObsRLModule] Detected MultiDiscrete action space, "
