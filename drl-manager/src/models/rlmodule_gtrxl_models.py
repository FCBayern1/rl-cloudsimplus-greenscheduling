...
def hierarchical_deterministic_logits(logits, num_slots: int, num_options: int):
    """P0-1 decode fix (V32B_ANNEAL_SPEC §0, Codex-approved 2026-08-17).

    The factorized gate emits per-slot 9-way NORMALIZED log-probs, but the
    evaluator's flat argmax then picks DEFER whenever p_hold > (1-p)*max_j q_j
    - an effective threshold of ~1/9 under diffuse routing (blind s2: 21,593
    consecutive all-defer steps). This transform rewrites ONLY the defer logit
    per slot so that flat argmax reproduces the hierarchical rule
        defer  iff p_hold > 0.5      (tie -> route, per spec)
        else   argmax(route[0:num_dc])
    Route logits stay untouched (their argmax is unchanged); recurrent state
    and connectors are untouched - the change lives purely in the emitted
    ACTION_DIST_INPUTS of the deterministic path.
    """
    B = logits.shape[0]
    z = logits.reshape(B, num_slots, num_options)
    p_hold = z[..., -1].exp()
    big = z.abs().amax(dim=-1) + 100.0
    new_defer = torch.where(p_hold > 0.5, big, -big)
    z = torch.cat([z[..., :-1], new_defer.unsqueeze(-1)], dim=-1)
