"""
RLlib PPO Training Script (New API Stack) using gMLP RLModules for Multi-DC MARL.

This mirrors `train_rlmodule_multidc.py`, but swaps the RLModule classes to
gMLP-based backbones.
"""
...
from src.models.rlmodule_gmlp_models import gMLPMaskedActionRLModule, gMLPDictObsRLModule
...
def create_rlmodule_gmlp_config(
        ...
        rl_module_spec = MultiRLModuleSpec(
            rl_module_specs={
                "global_policy": RLModuleSpec(
                    module_class=gMLPDictObsRLModule,
                ...
                "shared_local_policy": RLModuleSpec(
                    module_class=gMLPMaskedActionRLModule,
...
def train_rlmodule_gmlp(
    ...
    config = create_rlmodule_gmlp_config(
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train hierarchical multi-DC MARL with RLlib PPO + gMLP RLModules")
    ...
    if args.output_dir is None:
        ...
        args.output_dir = f"../../logs/{args.experiment}_gMLP_RLModule/{timestamp}"

    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    train_rlmodule_gmlp(env_config, global_model_config, local_model_config, training_config, str(output_dir))
