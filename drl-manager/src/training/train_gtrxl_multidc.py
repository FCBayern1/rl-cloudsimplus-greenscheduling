"""
RLlib Training Script with GTrXL RLModule for Multi-Datacenter MARL.

This script trains both the Global Agent and Local Agents using Ray RLlib
with the GTrXL (Gated Transformer-XL) RLModule for temporal modeling.

Key Features:
- GTrXL encoder with segment-level memory
- Gating mechanism for stable RL training
- Action masking for local agents
- Parameter sharing support
- Compatible with RLlib 2.5+ new API stack

Usage:
    python entrypoint_gtrxl.py --experiment experiment_multi_dc_10 --total-timesteps 100000
"""
...
from src.models.gtrxl_rlmodule import GTrXLMaskedActionRLModule, GTrXLDictObsRLModule
...
class GTrXLPettingZooEnv(ParallelPettingZooEnv):
    """Custom PettingZoo wrapper for GTrXL with RLlib API stack."""
...
def env_creator(config: Dict[str, Any]):
    ...
    return GTrXLPettingZooEnv(env)


def shared_policy_mapping_fn(agent_id, episode, **kwargs):
    """Policy mapping for parameter sharing."""
...
def create_gtrxl_config(
    env_config: Dict[str, Any],
    global_model_config: Dict[str, Any],
    local_model_config: Dict[str, Any],
    training_config: Dict[str, Any],
    gtrxl_config: Dict[str, Any],
    output_dir: Optional[str] = None
):
    """
    Create RLlib PPO configuration with GTrXL RLModules.
        ...
        rl_module_spec = MultiRLModuleSpec(
            rl_module_specs={
                "global_policy": RLModuleSpec(
                    module_class=GTrXLDictObsRLModule,
    ...
    config = (
        ...
        .training(
            ...
            lr=training_config.get("learning_rate", gtrxl_config.get("learning_rate", 1e-4)),
    ...
    logger.info("Successfully created PPO configuration with GTrXL RLModules")
    return config
...
def train_gtrxl(
    ...
    """
    Main training function using RLlib with GTrXL.
    ...
    logger.info("=" * 70)
    logger.info("RLlib Multi-Agent Training with GTrXL")
    logger.info("=" * 70)
    ...
    logger.info(f"GTrXL d_model: {gtrxl_config.get('d_model', 256)}")
    logger.info(f"GTrXL num_layers: {gtrxl_config.get('num_layers', 2)}")
    logger.info(f"GTrXL mem_len: {gtrxl_config.get('mem_len', 64)}")
    logger.info("=" * 70)

    # Register environment
    from ray.tune.registry import register_env
    register_env("multidc_gtrxl_env", env_creator)

    # Create RLlib config
    config = create_gtrxl_config(
        env_config,
        global_model_config,
        local_model_config,
        training_config,
    ...
    logger.info(f"TensorBoard logs: {output_dir}/multidc_gtrxl_training/PPO_*/")
    logger.info(f"Checkpoints: {output_dir}/multidc_gtrxl_training/PPO_*/checkpoint_*")
    logger.info("=" * 70 + "\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train hierarchical multi-DC MARL with GTrXL RLModule"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Memory length in steps (default: 64)"
    )
    parser.add_argument(
        "--dropout",
        type=float,
    ...
    # Get GTrXL config from experiment config or defaults
    ...
    if args.output_dir is None:
        ...
        args.output_dir = f"../../logs/{args.experiment}_GTrXL/{timestamp}"
