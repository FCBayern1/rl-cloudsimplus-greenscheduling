...
def test_disallowed_defer_goes_to_the_greenest_dc_with_room():
    acts, n, slots = route_disallowed_defers(
    ...
    assert acts == [2, 3, 1] and n == 1 and slots == [0]


def test_falls_back_to_the_greenest_dc_when_nothing_has_room():
    acts, n, _ = route_disallowed_defers([3], [0.0], [64], [0.1, 0.7, 0.3], [8, 8, 8], NUM_DCS)
    assert acts == [1] and n == 1


def test_mask_off_or_missing_arrays_change_nothing():
    acts, n, _ = route_disallowed_defers([3, 0], [1.0, 1.0], None, None, None, NUM_DCS)
...
def test_padding_slots_are_never_rerouted_nor_counted():
    ...
    acts, n, slots = route_disallowed_defers([3, 3, 3], [0.0, 1.0, 0.0], [4, 4, 0], [0.2, 0.9, 0.5], [8, 8, 8], NUM_DCS)
    assert acts == [1, 3, 3] and n == 1 and slots == [0]
