...
for A in oracle noforecast; do
  ...
  # clean eval always; oracle arm ALSO gets a shuffle (corrupted-forecast) eval
  # so the anti-phase run yields the full motivation triple:
  #   noforecast(clean) / oracle(clean) / oracle(shuffle).
  # noforecast has no forecast channel to corrupt -> clean only.
  PMODES="none"; [ "$A" = "oracle" ] && PMODES="none shuffle"
  for CK in $(ls -d ${RUN}checkpoint_* 2>/dev/null | sort -V | tail -3); do
    ckn=$(basename "$CK" | sed 's/checkpoint_0*/ck/')
    for PM in $PMODES; do
      tag=$([ "$PM" = "none" ] && echo clean || echo "$PM")
      lg=$OUTDIR/dc8ap_${A}_s${SEED}_${ckn}_${tag}.log
      BEFORE=$(pgrep -f "exe[.]edu[.]cspg[.]MainMultiDC" 2>/dev/null | sort -u)
      FORECAST_PERTURB_MODE=$PM DECODE_TOPK=0 \
      .venv/bin/python -m src.baselines.evaluate --experiment "$EXP" --global rllib \
        ...
        --output "$OUTDIR/eval_csv/dc8ap_${A}_s${SEED}_${ckn}_${tag}.csv" >"$lg" 2>&1
      cc=$(grep -a "Avg Carbon/MI" "$lg"|tail -1|grep -oE "[0-9.]+"|head -1)
      cf=$(grep -a "Avg Finished" "$lg"|grep -av Calling|tail -1|grep -oE "[0-9.]+%"|head -1)
      echo "[dc8ap ${A}_s${SEED} ${ckn}@ARGMAX ${tag}] cc=${cc:-?} completion=${cf:-?}" >>"$SUMMARY"
