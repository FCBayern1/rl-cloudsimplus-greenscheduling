...
def split_and_zero_pad(
    ...
    """
    def _is_batched_struct(x):
        """True when *x* is a nested struct (dict/tuple) whose leaves are BatchedNdArray."""
        if isinstance(x, BatchedNdArray):
            return False
        flat = tree.flatten(x)
        return len(flat) > 0 and any(isinstance(l, BatchedNdArray) for l in flat)

    def _struct_len(x):
        for l in tree.flatten(x):
            if isinstance(l, BatchedNdArray):
                return len(l)
        return 0

    def _slice_struct(x, start, end):
        return tree.map_structure(
            lambda s: BatchedNdArray(s[start:end]) if isinstance(s, BatchedNdArray) else s,
            x,
        )
    ...
    while len(item_list) > 0:
        ...
        # `item` is a nested struct (e.g. Dict obs) with BatchedNdArray leaves.
        elif _is_batched_struct(item):
            item_len = _struct_len(item)
            t = max_seq_len - current_t
            current_time_row.append(_slice_struct(item, 0, min(t, item_len)))
            if item_len <= t:
                current_t += item_len
            else:
                current_t += t
                item_list.appendleft(_slice_struct(item, t, item_len))
