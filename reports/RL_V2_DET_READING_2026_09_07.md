...
6. EU-CRD internals: **inactive where it matters.** Last-iteration statistics of both credit lines: `rho_forecast_mean` 0.000 and `r_forecast_abs_mean` 0.000 (the forecast responsibility channel carries no credit at all), `rho_routing_mean` 1.000 with standard deviation 0.000 (responsibility saturated on routing), `reweight_w_max` 1.000 with standard deviation 0.000 (the reweighting is uniform, i.e. a no-op), Δr mean −0.0002 with spread ±0.013, Δq mean −0.06 with spread ±1.1, τ 0.13. The ensemble and the Q-head are running, but the mechanism that is supposed to attribute error to the forecast never engaged.
...
- The gate-5 reading must be read together with gate 7: the forecast responsibility share was identically zero and the reweighting uniform throughout training, which matches the known ρ-saturation defect recorded for the v5 configuration. On this evidence the correct statement is "EU-CRD as configured behaved like vanilla with extra variance", not "credit assignment cannot help".
