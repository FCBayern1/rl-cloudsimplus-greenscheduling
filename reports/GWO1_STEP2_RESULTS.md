...
> ## ⛔ §1 的价值检查结论已于 2026-08-20 13:00 作废,不要引用
>
> **§1 报告的"放宽决策域有害(+25.65%)"是一个 bug 的产物,不是真实结果。**
> `TroughIndex.query()` 在槽外返回哨兵 `residual = 0.0`,使 `gate_flags` 里的
> `residual <= budget` 对每个在绿窗到达的作业恒真。宽门下 clairvoyant、naive、
> hazard 三个臂都在无条件延迟,那三格测的是"把绿窗到达的作业全部推走",
> 不是假设本身(5080 读代码发现,无需重跑即可定案)。
>
> 修复后在 PPO 判决底座上重测,**符号是反的**:
>
> | | 窄门 | 宽门 |
> |---|---:|---:|
> | clair vs nowait(3 锚中位) | −1.70% | **−2.60%** |
>
> 宽门优于窄门 −0.92%(3/3 同号),落在碳口径硬上界 −12.95% 之内。
>
> **另一处口径失效**:§1 全部数字在 `BACKLOG_CAP=200` 下取得,而该 cap 在
> ep79 顶死(blmax=200)。放大到 1000 后同格从 −21.40% 变为 −24.37%
> ——**cap=200 一直在压制 clairvoyant,所有 cap=200 的数字都是低估**。
> 同口径重算窄门池化应为 −19.15%,不是 §1 的 −17.99%。
>
> **§2(场景建成与认证)、§3(五个陷阱)不受影响,仍然有效。**
> 后续见 `GWO1_WIDE_FIX_AND_CEILING.md`。
