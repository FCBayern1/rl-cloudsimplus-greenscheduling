...
## Addendum B (2026-09-07, 02:50: the memory preflight failed; where the smoke runs)

Run 2 under Addendum A trained one line at a time and the fail-fast worked: NV alone still exhausted the 16 GB GPU (peak 15.33 GiB genuinely allocated, `expandable_segments` on, the last allocation to fail was 20 MiB), the runner aborted at once with `INVALID_SMOKE_RUN2_TRAIN_NV_EXIT_1_OOM` and no further training, reading or judgement ran. The memory is inherent to the interface: one training batch carries the candidate key and the legality mask at 8000 × 128 × 365 × 4 B = 1.49 GiB each, and the learner's forward and backward over 2048-sample minibatches of (128 slots × 5 sites × 73 offsets) logits add several tensors of the same order.

Per A1 the batch size, the architecture and the budget are not changed. The placement is not part of the recipe, so the order of attempts is: (i) a larger-memory GPU when one is reachable (Isambard, which needs the operator's SSH certificate; unreachable at 02:50); (ii) the same run with the learner on CPU, which changes no batch, no minibatch, no architecture and no budget, only the device, and is used only if its measured throughput allows the four lines to finish. The CPU option's throughput is measured on one PPO iteration before any line is started, and the measurement is recorded here. If neither is available the smoke waits; nothing is trimmed to make it fit.

**Addendum B measurement (2026-09-07, 03:13).** The CPU-learner probe (one PPO iteration of the NV block, everything else identical) completed 8000 env steps in 773 s with no memory problem, i.e. about 90 minutes per line and about 6 hours for the four lines. That is within the smoke's budget, so the amended run proceeds with the learner on CPU (`config_rl_v2_cpu.yml`, which differs from `config_rl_v2.yml` in `training.num_gpus` and in nothing else, verified key by key; manifest `manifest_cpu.json`, config sha da84e8abf845eeee). If Isambard becomes reachable before the run finishes, the same four lines may be rerun there from the frozen GPU config; whichever run completes first is the smoke, and the other is discarded without reading.
