...
- The learner is therefore not "the failure" in the sense of version 1 any more: version 1 failed on misaligned corpora; version 2 shows the learning target is already met by the rule the model starts from, and a learned correction can only preserve or degrade it. That is the answer to §0's open question: misalignment was the whole cause of the old failure; a learner is not needed to reach the expert on the truth key.
- F3 (diagnostic, TimeCAP key): cover_argmax keeps 0.20 pooled on these four windows (window by window −0.09, 0.64, 0.25, 0.03), against 0.82 on the six development windows; the fitted residual is worse (−0.44). The deployed forecast's error is severe on these windows and not learnable away by a small residual; this is consistent with the A2 STOP and does not override it.
...
- A further learner on the truth key is not warranted; on the TimeCAP key a learner would have to model the forecast's error structure, which the F3 residual did not manage with 420 decisions.
