...
- Candidates: the next windows of the same hash sequence up to position 24 (`draw_windows(52559, 24, "scene-interface-v1:2021:")`), frozen with their offsets and footprints in `stage_a_out/scene_v2_candidates.json` before any of them is read. Disclosed at the freeze: the 2021 file (52,559 rows) holds at most seventeen disjoint 2,922-row footprints, and after the pool of twelve the greedy sequence has room for exactly three more, offsets 49625, 36713, 30299 (positions 13–15); the candidate file's "positions 13–24" label is the ruling's wording, the list is these three. The search therefore covers three candidates, and "none passes → final STOP" applies to them.
