...
## Addendum B (2026-09-07, the sentinel fired; two harness defects fixed append-only; rerun)

The first sentinel run returned STOP_ROW_ALIGNMENT on all 36 (twin, window) pairs: the dumped forecast series had index h equal to the simulator's row t + h − 1 (max 940 W at h ≥ 1, index 0 exact). Cause: the option features are built before the forecast summaries step the provider, so the candidate key read the provider's cache of the previous step. Fix: the provider is refreshed at the observation's clock before the series is read (`_future_green_series`). The key recomputation then still differed by up to 0.036 on one site: the option features used `current_step`, which step() increments only after the observation is built, so the committed grid and the legality mask were read one row early relative to the executor's step (execution itself was and is start = t + κ + 1). Fix: the observation row is taken from the clock (`obs_row_from_clock`, row = clock − min_time_between_events; tested). After both fixes the smoke on window k0 gives series error 0.0 W, key recomputation error 0.0, starts = t + κ + 1 on every job, no masked or refused hold. No gate, window, label definition or model definition changed; the sentinel runs again from the start (commit a53db9af). Both defects also affected every earlier run of the offset twins' candidate key and mask by one row; the causal expert (own committed grid, wind-file curve) and the offline ladder are unaffected.

## Addendum C (2026-09-07, 02:00, model freeze before the test windows are read)

Sentinel rerun: SENTINEL_PASS on all 36 (twin, window) pairs (series error ≤ 3.1e-5 W, key recomputation exact). Labels: 18 windows × 35 = 630 decisions, 0 excluded states, 0 inconsistent (the expert's action is in the minimum-cost set every time), contracts green. Corpora: the expert's schedule replayed on both interface twins reproduces its carbon on all 18 windows. Fits (420 training decisions from the six development and six new training windows, 70 validation decisions), selection on the validation set loss over the frozen grid: F2 epochs 200, weight decay 1e-4, validation loss 1.151 (grid 3.17 / 3.17 / 2.13 / 2.10 / 1.20 / 1.15); F3 epochs 200, weight decay 1e-4, validation loss 3.270 (grid 3.55 / 3.55 / 3.38 / 3.38 / 3.29 / 3.27). Frozen: `fit/F2/residual.pt` 2cb6bee3a864cb89, `fit/F3/residual.pt` 512a75ca56e7afbe, feature function and decode `drl-manager/src/baselines/cover_residual.py` c0a9926b0a1f01c5, reader `f_v2_run.py` e9a3a73b8d03740b, windows record cef31b9b83b77d2f. The four test windows (24859, 28745, 41897, 7934) are read once after this commit.
