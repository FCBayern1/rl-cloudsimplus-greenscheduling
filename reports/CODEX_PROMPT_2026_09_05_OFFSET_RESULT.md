...
## 4. Ruling received and applied (2026-09-05 ~23:00)

Verdict maintained; wording narrowed to "the frozen discrete dispatch-offset action did not meet the cross-window robustness certification and the action line is terminated as preregistered" (not "cannot express forecast-driven scheduling"); the window criterion's weakness archived for future preregistrations only; gates 2 and 4 remain unread; the next proposal is a new prospective scene-and-interface design; machines idle until it is approved. Recorded in STAGE_D_PRIME_DESIGN §35. One read-only diagnostic (dense-grid replay of the offset oracle, scratch directory only) that had been started before the ruling is disclosed there and used only to inform the next design's grid choice.
