...
- Hardware: five Isambard-AI aip2 nodes, one seed per node, four lines per node, one GH200 each (`STAGE_D_PARALLEL_LINES=4`, `STAGE_D_GPU_MAP=NV:0,V:1,NE:2,E:3`). Hardware is still never tied to an arm, and a seed's four lines still complete in one hardware environment. Every seed runs on the same GPU model, as R-u requires; the model is GH200 instead of RTX 5080.
