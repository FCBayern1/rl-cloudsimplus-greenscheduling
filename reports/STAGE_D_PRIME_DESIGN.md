...
## 41. Ladder preregistration frozen; implementation (2026-09-06 02:40–03:3x)

Codex froze `reports/ERROR_LADDER_PLANNER_PREREG.md` at commit 24b5de60 (sha e0be0e95…, precedence body < A < B < C) with one notational clarification: the formula is written C_kg = J_int / (3.6 × 10^11), the denominator bracketed. Implemented and committed before any carbon:
- `HostProfile` comment corrected (65.64 W per 32-PE job, MIPS-based utilisation); `./gradlew -q compileJava` clean.
- `ladder_planner.py`: exact CP-SAT model (x[j, d, s], per-(site, step) capacity, load 2020 mW per PE, active hosts h ≥ PEs/64 with a 1000 mW floor, brown ≥ draw − G_mW, objective Σ(49·brown + draw) ≡ Σ(50·brown + green), OPTIMAL required), model settlement on any curve, quantisation bound, factor preflight; tests: unit lock, 65.64 W settlement, green-window optimum with deadline and capacity, dominance (truth optimum never beaten by three wrong-curve optima in model settlement).
- `schedule_replay` arm: a fixed (site, start) schedule replayed through the offset executor as (site, κ); late or masked pairs counted, never silently clipped; tests.
- `ladder_run.py`: truth curves and job facts from the simulator's own dump on the dev DEFER twin (first sighting, PEs, MI, time to deadline); rung curves (shrink around the site's full-year 2021 mean under the simulator's kW × 1000 / divisor conversion, seeded shuffle, anti); solve truth first, the other six only after truth closure; replay on the dev OFFSET twin; per-rung closure (3 %, per-job site and start, counters) and gates L1/L2; tests.
Stage 1 launched: dump → truth solve → truth replay → truth closure; no other rung's carbon is produced before it closes.

## 42. Overnight queue under the user's authority (2026-09-06 02:10–03:00), and the CP-SAT → HiGHS ruling

CP-SAT stage-1 run 1 was invalid (dump carried the normalised deadline; `ttd_sec` added); run 2 (default CP-SAT) was interrupted on the Codex ruling that produced Addendum D (HiGHS as the only judging solver, compound OPTIMAL, CP-SAT cross-check only, one solve at a time, 600 s HiGHS wall clock, no retries): recorded INCOMPLETE_INTERRUPTED (k3 had proven OPTIMAL in 294 s), never a STOP. HiGHS truth stage launched at 02:03 on an otherwise idle machine (gradle daemon stopped, no evaluation or training) into `stage_a_out/ladder_v2/`.

Queued to run unattended, strictly in sequence, each zero-RL and either frozen or a required check: (1) if all six truth rungs are OPTIMAL and closed on replay → the other six rungs, replay, gates L1/L2, archive to `reports/manifests/ladder_v2/run1/`; any STOP is recorded, never retried; (2) Addendum D4 CP-SAT cross-check on the six truth rungs (never concurrent with a formal HiGHS solve); (3) the scene design's frozen steps 2b/2c (margin probe, P0′) on the six development windows, archived for the morning review and used for no decision; (4) the interface smoke of §4.4 / A4 (dense float32 first) on window k0. Implemented meanwhile, tested and committed, no run: the candidate-aligned key `cand_green_cover` (energy-weighted residual green after static draw and the executor grid's committed load, the planner's power model, horizon 121 steps, computed from the arm's own future series through one path: provider rows for TimeCAP / perturbed arms, the Java oracle for godeye, persistence for blind), the interface config twins, and the smoke script. RL is not started: it needs its own preregistration after the ladder's load-bearing rung is known.

## 43. D-stage truth result and Addendum E (2026-09-06 02:30–02:35)

Under Addendum D the truth rung proved OPTIMAL on k0 (HiGHS, 337 s, J = 222,043,518, C_model 0.000617 kg) and reached 600 s unproven on k1 (and k2, run before the ruling arrived): STOP_SOLVER_RUNG_UNRESOLVED, ruled to mean "a 600 s budget is insufficient", not a scene or ladder failure; cells after k1 are post-STOP diagnostics; the stage is archived with its terminal log as `reports/manifests/ladder_v2/d_stage_stop/` (the stage code wrote per-cell JSON only for OPTIMAL cells, so k1's numbers live in the summary; corrected below). Codex note: `CODEX_PROMPT_2026_09_06_LADDER_TRUTH_STOP.md`.

Addendum E (Codex; committed fee56221, sha c47ccfb5…): one uniform HiGHS limit of 3600 s per (window, rung), `mip_rel_gap` 0; model, seven rungs, compound OPTIMAL, closure and judgement gates unchanged; fresh directory `ladder_v3`; every solve writes its JSON atomically whatever its outcome; a cell unproven at 3600 s stops the ladder with no retry, no extension, no CP-SAT switch and no Addendum F; the six truth rungs and their replay closure precede any other rung; the cross-check, margin probe, P0′ and interface smoke may run but never concurrently with a formal solve. Implemented (66618bca) and queued for the night in this order, each waiting for the previous to end: D-stage archive → CP-SAT cross-check (D4) and scene steps 2b/2c → interface smoke → the formal E stage (dump, six truth solves, replay, closure, then the other six rungs, replay, judgement, archive to `reports/manifests/ladder_v3/run1/`).

## 44. Addendum E stage: STOP on two counts (2026-09-06 04:03–05:12, read 05:15)

Solver rung: k0 OPTIMAL (340 s, J 222,043,518); k1 FEASIBLE at 3600 s (incumbent 371,701,681, bound 289,220,971, gap 22.7 %, 46,817 nodes) → STOP_SOLVER_RUNG_UNRESOLVED; k2–k5 never solved. The D4 cross-check on the D-stage cells shows the two solvers proving different windows (HiGHS k0, CP-SAT k3 in 349 s) and neither closing k1. No retry, extension or solver switch (E4/E5).

Closure: the stage's k0 replay was invalid because the harness had not set the every-step grid (OFFSET_GRID_DENSE=1), so the simulator's 12-value dyadic mask and the replay arm's 73-value action index disagreed (26 "masked" holds, jobs on the wrong sites, C_sim 22× C_model). Harness fixed with tests (`replay_env`; the replay arm refuses a grid mismatch; the judge reports never-solved windows as unresolved instead of a KeyError). Corrected k0 replay: all 35 jobs on the planned site at the planned start, counters zero, but C_sim 0.000655 vs C_model 0.000617 kg (+6.19 %, tolerance 3 %): simulator draw +1.5 % (30.90 vs 30.43 Wh), brown +11 % (0.706 vs 0.638 Wh); brown is ~2 % of draw, so the composite tolerance is a tolerance on a small residual. STOP_PLANNER_CLOSURE_RUNG on k0 independently of the solver stop.

Queue results before the stage: scene 2b margin 2 steps / forced 0 / ontime 1.0 on six windows; 2c P0′ PASS_P0_PRIME; interface smoke PASS_DENSE_F32 (throughput 0.996, RSS 1.000). Archive `reports/manifests/ladder_v3/run1/` (README states both stops and the replaced replay). Per Addendum E no Addendum F; the ladder as preregistered is closed. Codex note `CODEX_PROMPT_2026_09_06_LADDER_E_STOP.md` asks whether a new preregistration is admissible and what its two design facts are.

## 45. Settlement diagnostic on the fixed k0 schedule (2026-09-06, Codex-approved, no verdict)

Codex closed the ladder (no Addendum F) and approved one diagnostic: align model and simulator per second and per DC on the fixed k0 schedule, four probe levels, attribute the 0.47 Wh. Done: `reports/SETTLEMENT_DIAG_2026_09_06.md`, code `settle_probe.py` (+ tests), artefacts `reports/manifests/settle_diag/`. The gap is fully attributed: A one host per job in the simulator vs ⌈PEs/64⌉ in the model (+0.206 Wh, 740 host-steps; probe L2); B one extra 1-s power sample at the end of every job that starts while its site is mid-cycle (+0.269 Wh, 15 job-steps; rule exact on 35/35 k0 jobs; probes L1 no / L3a, L3c yes); C harness: the planner's truth curve is hold-last extrapolated beyond the dump's 599 rows while the replay ran to 638 (+0.012 Wh brown); D DC2 is a 128-PE RS700A while the model uses 64-PE hosts (−0.005 Wh). Brown: B +0.047, C +0.012, A +0.008 of the 0.068. Per-step samples sum to the ledger exactly, starts are exact. Probe L3b was a probe artefact (start > 73 rows after sighting is clipped by the replay arm) and is not used. Nothing was changed under the frozen rules; decisions A–D and any new preregistration are with Codex (`CODEX_PROMPT_2026_09_06_SETTLE_DIAG.md`).

## 46. Ruling A–D applied under the user's direct authority; closure exact (2026-09-06, 09:30–12:30)

Ruling (user, with Codex's text): A the model follows the simulator's placement; B source-level trace before any modelling; C and D mandatory; then engineering closure without re-solving; then a solver feasibility gate (k1 must be proven) before any new preregistration. The user then handed the day over ("按既定目标干活,不要问我").

B, source level (per-step trace of the gateway, new `SETTLE_TRACE_CSV`, plus `CloudletBoundaryTraceTest` on pure CloudSim Plus 8.5.5): a cloudlet submitted while another is processed on the same datacenter gets its last 401 MI (0.01 s) processed one whole second late because the datacenter's scheduling interval is 0 (updates only at estimated finishes), `updateHostsProcessing` clamps the next delay to `minTimeBetweenEvents + 0.01`, and `isTimeToUpdateCloudletsProcessing` refuses updates within `minTimeBetweenEvents` (1.0 s) of the last; the pure-CloudSim test gives 48 s / 49 s with interval 0 and 48 s / 48 s with interval 1.0. Classified as a discrete-update artefact (Codex's third case), fixed by aligning every datacenter's processing to the 1 s step in the certification twin (`datacenter_scheduling_interval`, new key, default 0 = legacy). A first attempt with min_time_between_events 0.01 was discarded: it shifts the clock of every observation by 0.99 s and every start by one row (the +1 of the wind-row mapping is that constant).

C: `truth_curve` from the wind files, row = offset + tz + t + ⌊min_time_between_events⌋ + 12 (the provider drops 12 rows in COMPRESSED mode); exact on all six windows; signatures both sides, per-row assertion at closure, STOP past the file. D: per-site RS500A/RS700A profiles (65,640 / 65,600 mW per job). A: VM i on host i mod H, lowest free VM, freed one row after its job ends; theorem and property test; premise occupancy(d, t) = running + ending ≤ H_d as a planner constraint (linear, no host variables) and a settlement check; `placement_hosts` reproduces the simulator's VM and host for 35/35 k0 jobs. LAG: the route-now path starts a job at sighting + 2, held jobs land exactly; the planner's earliest start is now sighting + 2 (the only residual of the first closure: two route-now jobs one row late).

Engineering closure on the certification twin (`config_ladder_cert_*.yml`, `ladder_v4` dumps): probes L1/L2/L3a/L3b/L3c exact (model draw == simulator draw, 48 s spans); k0 archived v1 schedule: draw identical, carbon +1.48 % (passes 3 %, but the LAG residual is visible and attributed); k0 v1 schedule with the two route-now jobs at sighting + 2: carbon relative error 4.3e-7, max per-step power difference 1.2e-5 W, every counter zero, curve rows match. Archive `reports/manifests/settle_diag/{closure,probes_v2}`; report §6 of `SETTLEMENT_DIAG_2026_09_06.md`. Solver feasibility gate launched: six truth rungs, model version 2, HiGHS 3600 s, gap 0, on `ladder_v4`; gate = k1 proven.

## 47. Solver gate passed by equivalent tightening; preregistration v2 frozen (2026-09-06, 13:00–14:20)

The plain version-2 formulation proved k0 in 2707 s (40,552 nodes), slower than version 1 and hopeless for k1. Equivalent tightening (Codex's preferred route): on every cell whose candidate jobs draw the same P the convex envelope of brown = max(0, P·n − G) over integer job counts, brown ≥ (P(m+1) − G)(n − m) with m = ⌊G/P⌋, valid for every integer solution (RHS − true brown = (Pm − G)(n − m − 1) ≤ 0) and unit-tested for equivalence on random instances. With it every truth rung is root-node optimal in 3.7–4.4 s (k0's optimum unchanged, J 226,384,249; k1 proven, J 370,233,055). Truth closure on the certification twin: TRUTH_CLOSED on all six windows, carbon relative error ≤ 2.8e-7, draw ≤ 3e-15, brown ≤ 4.7e-7 Wh, curve rows matching; the judge now gates draw (0.1 %) and brown (0.002 Wh) separately. Archive `reports/manifests/ladder_v4/gate`.

Preregistration v2 (`reports/ERROR_LADDER_PLANNER_PREREG_V2.md`, sha256 b1a723a950f2a93c) frozen at commit 6d9d1e64 under the user's authority for the day; version 1's two STOPs stay on record; no non-truth rung was solved or read before the freeze. The remaining six rungs, all replays and the judgement were launched immediately after the freeze (`ladder_v4`, archive `reports/manifests/ladder_v4/run1`).

## 48. Ladder v2 read: LADDER_READ, every rung load-bearing (2026-09-06, 14:25)

All 42 cells root-node optimal (3–5 s), all 42 replays closed (worst carbon relative error 5.4e-7), six of six windows valid at L1 (the truth planner removes 73–92 % of the flat schedule's carbon, headroom 0.0016–0.0059 kg), and every rung load-bearing at L2 with the harm on 100 % of the headroom: pooled loss vs truth 41.5 % (λ 0.75), 211 % (0.5), 444 % (0.25), 465 % (flat), 419 % (shuffle), 496 % (anti). Reading in `reports/LADDER_V2_READING_2026_09_06.md`. Forecast quality is load-bearing for the certified exact planner on this scene; nothing is claimed about a learned policy or the deployed TimeCAP error (A2 stays STOP). Next: F1–F3 causal fits on the truth schedules, then an RL preregistration on the certification twin (its own freeze before training); the 2020 windows stay sealed.

## 49. User ruling: a causal bridge before F1–F3; causal expert preregistered and frozen (2026-09-06, afternoon)

The user (with Codex's text) accepted the ladder as the first link and reordered the chain: offline exact ladder → causal rolling expert and causal error gate → F1–F3 with the expert's decisions as labels → RL preregistration → vanilla / EU-CRD; 2020 sealed throughout. Reason: the offline planner knows every future job; the value RL can reach is the value an expert with only arrived jobs can reach. Built: `causal_expert` arm (rolling exact MILP over the newly visible jobs on the committed reservation grid with the arm's own curve, executed through the dense offset executor; `causal_decide` pure core, tests), planner support for committed load and per-site masked starts (tests), runner `causal_run.py`. Preregistration `reports/CAUSAL_EXPERT_PREREG.md` frozen at commit 6f7b5807: gate A capture ≥ 0.80 pooled and ≥ 0.70 on ≥ 5/6 windows against the ladder's C_flat / C_truth; gate B λ = 0.75 harms the expert on ≥ 5/6 windows and ≥ 5 % pooled; contract green; unsolved steps void a window. Truth arm launched after the freeze.

## 50. Causal gate A passed; F1–F3 preregistered (2026-09-06, evening)

Gate A: the causal expert (rolling exact MILP, only arrived jobs, 35 proven-optimal decisions per window, no unsolved step, contract green) reaches pooled capture 0.968 of the offline planner's headroom, every window ≥ 0.918 (gate 0.80 pooled, 0.70 per window on ≥ 5/6). The value the ladder certified is reachable with the information an online policy has. The six error rungs of the expert are running for gate B. F1–F3 preregistration (`reports/F_FITS_PREREG.md`) frozen at f81328ca, conditional on CAUSAL_READ: labels = the expert's (site, κ) decisions; corpora F1 (offset twin), F2 (interface twin, candidate key from truth), F3 (interface twin, candidate key from TimeCAP) produced by replaying the expert's schedule with the observation dump; frozen C4 recipe; primary gate executed capture ≥ 0.50 on the held-out windows k4, k5 with the contract green; the classification gate reported with the wait class κ ≥ 2 and expected INVALID (3 of 70 held-out decisions at κ = 1).
...
## 54. The RL-side forecast channel was 12 rows stale on every SPLINE scene (2026-09-06, 22:40)

While analysing why the causal expert's choice had the key's maximum cover in only 50 of 210 decisions, the candidate key was recomputed from the truth curve: it matched the truth window starting about 13 rows earlier. Direct check of `PerturbedGodEyeProvider` built as the env builds it: its index 0 at clock c is the wind-file row tz + episode offset + c, while the simulator burns row tz + episode offset + c + 12 (the Java provider's COMPRESSED SPLINE skip, GreenEnergyProvider.loadCsvData). Exact: provider index i = simulator obs row t + i − 12 (max difference 0.000 W). The env's future series then placed provider index h − 1 at future step h, one more row. So every `dc_future_*` summary and every `cand_green_cover` produced through the Python providers (perturbed_godeye, TimeCAP) on every COMPRESSED SPLINE scene, including Stage D, the RL step-2 pilot and the interface twin, described the weather 12–13 rows before the one the simulator was burning; the "godeye" tier was not clairvoyant. The Java-oracle path (`green_oracle_mode: godeye`) was aligned. Fix (append-only, tested): `simulator_row_shift` adds the 12-row skip to the per-DC offsets in COMPRESSED SPLINE mode, and `future_series_from_raw` maps future step h to provider index h. Consequence for earlier RL readings: any conclusion that a policy "ignored the forecast" or that the forecast was "not load-bearing for RL" on those scenes was drawn with a stale forecast channel and is not reusable as evidence; the offline ladder and the causal expert never used this channel (wind files directly) and stand. The F2/F3 corpora and the cover_argmax diagnostic are being rebuilt on the aligned key.

Result on the aligned key (23:30): cover_argmax with the truth key captures 1.009 pooled (0.986–1.046 per window): the zero-parameter rule equals the causal expert. With the TimeCAP key 0.820 pooled (0.435–0.946). The F1–F3 fits had been trained on a key 13 rows stale relative to their labels; their frozen verdict stands as produced, its interpretation is corrected in the reading.

## 55. F_FITS_V2 frozen and launched (2026-09-07)

User ruling: F v2 with a candidate-shared scorer (score = cover + residual, zero residual ≡ cover_argmax bit for bit, lexicographic ties outside the argmax), true candidate costs as labels (single job: exact increments; several jobs: fix-and-resolve; excluded states counted), twelve hash-drawn windows (6 train, 2 validation, 4 sealed test; old six into training), model selection on the validation set loss only, F2 hard gate (pooled capture ≥ 0.50, per valid window ≥ cover_argmax − 0.02, ≥ 3/4 valid by the ladder's L1 rule), F3 diagnostic, a row-alignment hard sentinel before any corpus, GPU parked, 2020 sealed. Preregistration `F_FITS_V2_PREREG.md` frozen at 80b693fb; Stage D's forecast readings withdrawn as stale-forecast legacy (STAGE_D_PREREG.md Addendum J). Addendum A before the draw: 2021 has no position 2922 rows from every read window (capacity 0), 2022 is a stub, 2020 sealed; an episode touches about 1007 rows; footprint 1200 (capacity 22). Draw (tag f-v2:2021, record db80a2a324755a63): train 38088, 5463, 11249, 44834, 31713, 35380; validation 21850, 1839; test 24859, 28745, 41897, 7934. Implementation (139f3834): `candidate_costs` (tested against the joint solver), labels written by the causal expert, `cover_residual.py` (features, decode, set loss, grid selection), `cover_residual` arm, sentinel side channel, `f_v2_run.py`. Stage 1 launched: sentinel → labels → corpora → fits.

## 56. The F_FITS_V2 sentinel fired and found two more one-row defects (2026-09-07, 00:30–01:30)

STOP_ROW_ALIGNMENT on all 36 pairs at the first run. (1) The candidate key was built from the forecast provider's cache of the previous step (index h = row t + h − 1); fixed by refreshing the provider at the observation's clock before the key. (2) The option features (legality mask, candidate key's committed grid, held summaries) used `current_step`, which is one behind the observation's row because step() increments it after building the observation; fixed by taking the row from the clock (`obs_row_from_clock`). After the fixes: series error 0.0 W, key recomputation exact, executor starts exactly t + κ + 1. F_FITS_V2 Addendum B records both; the sentinel reruns from the start. Every earlier offset-twin run had its key and mask one row early (execution was right); the causal expert and the ladder never used them.

## 57. F_FITS_V2 read: STOP_F2_LEARNER; the cover rule is the expert (2026-09-07, 02:15)

Sentinel PASS (36/36), 630 exact-cost labels (0 excluded, 0 inconsistent), corpora exact, F2/F3 residuals selected on validation loss and frozen (4a552fb4). Four sealed test windows all valid (headroom rel 0.44–0.90), closures ≤ 2e-7, contracts green everywhere. cover_argmax on the truth key captures 1.000–1.016 of the causal expert's headroom on windows it never saw; the fitted residual 0.910–0.988 (pooled 0.968 ≥ 0.50, but below cover_argmax − 0.02 on all four) → STOP_F2_LEARNER as preregistered. F3 (TimeCAP key): cover_argmax 0.20 pooled (−0.09 … 0.64), residual −0.44. Reading `reports/F_FITS_V2_READING_2026_09_07.md`. Interpretation: with the aligned interface the greedy cover rule is the online expert; a learned residual can only preserve or degrade it on the truth key; the old F failure was the misalignment. RL design should start from the cover rule with cover_argmax as the reference on every window and tier; GPU parked, 2020 sealed.

## 58. RL_V2 health smoke preregistered and launched (2026-09-07, 03:40)

User ruling after STOP_F2_LEARNER: behaviour cloning closed (F_FITS_V2 sealed); the RL policy is a fixed cover prior (buffer, never trained) plus a zero-initialised residual; four lines NV / V / NE / E (persistence cover vs own forecast; vanilla vs EU-CRD) with one architecture, prior, action, budget and seed; the untrained module's deterministic decode must reproduce cover_argmax (index ties) action for action and carbon for carbon; cover_argmax reported at every tier; one seed of 56k steps first; GPU parked for long runs; 2020 sealed. Two wording corrections applied to the F v2 reading (misalignment voids v1, not proven the sole cause; the TimeCAP "not learnable" claim withdrawn). Preregistration `reports/RL_V2_SMOKE_PREREG.md` frozen at 96256505 with: `cover_prior_fixed` in the score module (tested: init logits equal the cover on legal candidates, argmax equals cover_argmax with index ties), provider tiers shrink75/50/25/0 tested equal to the ladder's rungs, `gen_rl_v2.py` (four blocks by whitelisted diff from the certification interface twin, eval blocks per tier, manifest), reading windows = the F_FITS_V2 validation and test windows (never trained on), training allowlist = the six development plus six F_FITS_V2 training windows. Scripts 12b80734. Smoke launched on the local GPU.

## 59. RL_V2 run 1 invalid (CUDA OOM); Addendum A and run 2 (2026-09-07, 02:30-02:45)

Run 1 died before any reading: NV and V trained two at a time on the 16 GB GPU and both hit CUDA out of memory at their first PPO learner update (1.52 GiB = one training batch of the candidate key, 8000 x 128 x 365 x 4 B; the other learner held 12.93 GiB). The runner had no fail-fast and started NE and E, stopped by hand. Archived as INVALID_SMOKE_RUN1_OOM (`reports/manifests/rl_v2/invalid_run1_oom/`), no number used.

User ruling, frozen as Addendum A (1f13d584): (A1) fail-fast runner, one line at a time, the first line is the memory preflight, a larger GPU rather than a smaller batch if one line does not fit; (A2) the fixed cover prior must be carried by the SAMPLED policy, not only by the argmax: at gain 1 the softmax over ~355 legal candidates of cover in [0, 1] samples an action whose cover is 0.478 of the best on average (measured on the twelve training windows, no carbon involved), so the logits become `cover_prior_gain x cover + residual` with the gain frozen as the smallest of {1, 5, 10, 20, 40, 80, 160, 320} reaching a mean sampled-cover ratio of 0.95 → 20 (measured 0.478 / 0.769 / 0.887 / 0.952); a new hard init gate requires, with the registered stochastic decode on the reading windows, a mean sampled-cover ratio ≥ 0.95 and a pooled carbon ≤ 1.05 x cover_argmax's, else STOP_INIT_PRIOR_NOT_CARRIED; (A3) only execution faults (OOM, missing files, crashes, wiring) may be fixed and rerun, gates 3–7 are substantive STOPs. Implemented and tested (440c539c): gain in the module and both model-config builders, configs regenerated (sha 2bb292f96fdf8075), runner with return-code checks, sequential training, an init-stochastic pass and both init gates as aborts, judge extended. Run 2 launched.

## 60. RL_V2 smoke: gate 1 passed, gate 1b STOP; ruling = deterministic deployment (2026-09-07, 12:16-13:15)

All four lines trained to 56 000 steps on the CPU learner (NV 87 min, V 88, NE 180, E 182; the middle two shared the machine with the reference arms). Three wiring faults were fixed append-only along the way and are on record: the last-checkpoint glob was one level too shallow (NV was declared checkpoint-less after a complete training), the init-checkpoint glob one level too deep, and the reference subshell called a relative interpreter after changing directory, so the 48 reference runs silently never started; a fourth self-inflicted fault (editing the script while bash was executing it) truncated one run's post-training phase. None of them touched a definition; every affected step was repeated, and completed lines are now skipped by a resume check.

Gate 1 INIT_OK: the untrained module's deterministic decode reproduces `cover_argmax` on 210/210 decisions per line and its carbon exactly, on all four lines. Gate 1b STOP_INIT_PRIOR_NOT_CARRIED: the sampled policy's mean cover ratio is 0.961-0.984 (bar 0.95, met) but its pooled carbon is 1.112-1.323 x the same rule decoded deterministically (bar 1.05, missed), contracts green; in capture terms the stochastic init reaches 0.938 (V) and 0.930 (E) against `cover_argmax`'s 1.010. The gate measured the cost of exploring among many near-equal-cover actions, not a broken prior.

User ruling: the smoke is closed as STOP_INIT_PRIOR_NOT_CARRIED and never rewritten; gate 1b is kept as a diagnostic of stochastic exploration cost; the gain is not raised, the bar is not relaxed, and the reference is not swapped for each line's own noisy init. Training keeps stochastic sampling; deployment evaluation becomes deterministic. The four checkpoints are reused with their hashes frozen first (`checkpoint_hashes.json`), and gates 3-7 are re-registered under deterministic decode in `reports/RL_V2_DET_PREREG.md`, read once. If that passes, the multi-seed experiment is registered from the outset as stochastic training with deterministic deployment, new seeds, sealed 2020.

## 61. RL_V2 deterministic deployment read: FAIL on gate 5, EU-CRD's forecast credit was inert (2026-09-07, 18:03)

96 readings, six windows never trained on, deterministic decode, contracts green everywhere, checkpoint hashes verified against the record. Gates 1-4 and 6 pass: the untrained module reproduces `cover_argmax` exactly; 56 000 PPO steps leave the prior intact (V clean capture 1.002 against the rule's 1.010, i.e. 0.992 of it); the shrink ladder hurts the trained policy monotonically and slightly more than it hurts the rule (V +27.7 % at λ = 0.75 against the rule's +19.5 %, up to +210 % at anti); E's clean carbon is 58.8 % below NE's with an action-marginal KL of 0.114 between godeye and shrink 0.75, so it is not winning by ignoring the forecast. Gate 5 fails: E's λ = 0.75 loss is 30.9 % against a bar of 13.8 %, and E is worse than V at every rung except shuffle. Gate 7 qualifies it: `rho_forecast_mean` and `r_forecast_abs_mean` are identically 0, `rho_routing_mean` 1.0 with zero spread, the reweighting uniform (`reweight_w_std` 0) - the forecast-credit channel never engaged, matching the recorded ρ-saturation defect of the v5 configuration. Reading `reports/RL_V2_DET_READING_2026_09_07.md`, archive `reports/manifests/rl_v2/det`. Substantive failure: the line stops for a ruling, nothing tuned; the correct statement is that EU-CRD as configured behaved like vanilla with extra variance, not that credit assignment cannot help.
