...
cd $REPO/drl-manager
declare -A EXPS
EXPS[oracle]=experiment_v2026_gamble_oracle
EXPS[noforecast]=experiment_v2026_gamble_noforecast

echo "===== V2026-GAMBLE-FV seed $SEED start $(date '+%m-%d %H:%M') =====" >>"$SUMMARY"
for A in oracle noforecast; do
  EXP=${EXPS[$A]}; OD=$REPO/drl-manager/logs/v2026gb_${A}_s${SEED}
  ...
  .venv/bin/python entrypoint_rlmodule_gtrxl.py --config "$REPO/config_C.yml" \
    ...
    --output-dir "$OD" > "$OUTDIR/v2026gb_${A}_s${SEED}_train.log" 2>&1
  ...
  for CK in $(ls -d ${RUN}checkpoint_* 2>/dev/null | sort -V | tail -3); do
    ...
    lg=$OUTDIR/v2026gb_${A}_s${SEED}_${ckn}_clean.log
    ...
    .venv/bin/python -m src.baselines.evaluate --experiment "$EXP" --global rllib \
      ...
      --output "$OUTDIR/eval_csv/v2026gb_${A}_s${SEED}_${ckn}_clean.csv" >"$lg" 2>&1
...
echo "V2026-GAMBLE-FV seed $SEED DONE $(date '+%m-%d %H:%M')" >>"$SUMMARY"
