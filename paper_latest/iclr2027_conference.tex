...
\end{wrapfigure}
Forecast quality is not stable over the lifetime of a deployed system: it degrades silently as sensors malfunction, models go stale and data pipelines fail \citep{polasekPredictingPhotovoltaicPower2023, meenalWeatherForecastingRenewable2022}. Degradation is not only a loss of accuracy. Work on adversarial attacks against load and solar forecasting shows that a pipeline's output can be steered in arbitrary directions by small perturbations of its weather inputs, while the inputs still look like weather and the output still looks like a forecast \citep{chenExploitingVulnerabilitiesLoad2019, tangAdversarialAttacksSolar2021}. That combination, a plausible signal whose relationship to the realised supply has broken, is the failure this work targets. Its adversarial instance is the harder case and is left open here. Its mundane instance needs no attacker, since a misconfigured site-to-feed mapping delivers one site's coherent and internally consistent forecast to another while every value stays in range. Ordinary forecast noise is a different thing and not the target. A policy trained on noisy forecasts has already priced that noise into its returns, and the unconditional trust this paper is about is what training on a mostly accurate signal produces rather than something noise can break. What breaks it is a shift in the channel that training never saw, where the statistics survive and the correspondence does not. A corrupted forecast in that sense is no longer a noisy reference but a confident error. In Section~\ref{sec:experiments} a well-trained policy acts on such forecasts without hesitation, routing work toward datacentres it wrongly believes are about to turn green and emitting more carbon than a policy that ignores the forecast altogether (Figure~\ref{fig:motivation}): the forecast has become a liability.
