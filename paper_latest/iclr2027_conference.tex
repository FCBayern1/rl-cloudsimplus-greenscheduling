...
\ifFragileNone
Every seed in both arms holds completion inside the contract under greedy decoding, so the column
records no failure to explain.
\else\ifFragilePattern
\ResFragileCount{} over-defers under greedy decoding in both arms, a property of the training recipe
rather than of either method, and only EU-CRD makes it observable, its critic-ensemble disagreement
rising before any conventional metric moves (Appendices~\ref{app:failure} and \ref{app:mechanism}).
\else
A single seed over-defers under greedy decoding. One instance does not establish a pattern, so it is
reported as an isolated case in Section~\ref{sec:limitations} rather than analysed as a failure mode.
\fi\fi
