...
> 本 README 以“当前代码实际可运行的入口”为准（`drl-manager/entrypoint.py`、`drl-manager/entrypoint_pettingzoo.py`、`cloudsimplus-gateway` 的 `Main`/`MainMultiDC`）。

---

## 快速开始（推荐：多数据中心 + RLlib + PettingZoo）
...
> 如果你已经按“多 DC”装过 `requirements_rllib.txt`，这一步可以跳过（SB3 依赖已包含）。
...
如果你想跑其它 single-dc 实验，把 `--exp`/`EXPERIMENT_ID` 换成 `config.yml` 里的实验名即可（例如 `experiment_1_new`、`experiment_2`、`experiment_3` 等）。
