"""TB13-v3 zero-emissions preflight. Reads the frozen cohort and nothing else.

Kept apart from `preflight_v3.py` on purpose: that file's digest is already stamped into
the completed Round 0-v3 manifest, and a finished run's provenance should not move because
a later stage was added to the same file.
...
GUARDED_SOURCES = ("workload_v3.py", "schedule_feasibility.py", "zero_emission_v3.py")
...
def load_cohort(round0_dir):
    ...
    path = os.path.join(round0_dir, "cohort_v3.json")
    ...
    manifest = json.load(open(os.path.join(round0_dir, "round0_v3_manifest.json")))
    ...
    return cohort, {"cohort_sha_recorded": recorded, "cohort_sha_derived": derived,
                    ...
                    "manifest_sha_matches": manifest["cohort_v3.json"] == on_disk}
...
def key_of(cell):
    p = cell["physical"]
    return w3.workload_key(0, p["horizon"], p["pes_per_job"], p["concurrency"],
...
def main(round0_dir=None, out_dir=None):
    ...
    out_dir = out_dir or os.path.join(HERE, "zero_emission_v3_out")
    ...
    for c in cells:
        ...
        if acc is None:
            ...
            wl = acc["workload"]
            budget = w3.budget_for(wl, c["budget_fraction"])
            assign, spent = sf.reservation_edf(wl, budget)
            row["budget"] = budget
    ...
    touched = set()
    for c in cells:
        touched |= r3._turbines(c["physical"])
    ...
    dirty = subprocess.check_output(["git", "-C", REPO, "status", "--porcelain", "--",
                                     "g1/tb13/zero_emission_v3.py",
                                     "g1/tb13/workload_v3.py",
    ...
    r0._atomic_write(os.path.join(out_dir, "zero_emission_v3_cells.jsonl"),
                     "\n".join(json.dumps(r, sort_keys=True) for r in per_cell) + "\n")
    r0._atomic_write(os.path.join(out_dir, "zero_emission_v3_summary.json"),
