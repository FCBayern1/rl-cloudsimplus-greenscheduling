"""Round 1-v3: freeze one blind arm on the cohort, then measure against the exact model.
...
MIN_PES_SHARE = ig.MIN_PES_SHARE
TIME_LIMIT_S = 30.0
OUTER_WORKERS = 4
TRACKED = ("g1/tb13/round1_v3.py", "g1/tb13/round0_v3.py", "g1/tb13/workload_v3.py",
           "g1/tb13/zero_emission_v3.py", "g1/tb13/causal_blinds.py",
...
def preflight(round0_dir, zero_dir):
    ...
    dirty = subprocess.check_output(
        ...
        text=True).strip()
    if dirty:
        raise RuntimeError("refusing to run Round 1-v3 from a dirty tree:\n" + dirty)
    cohort, integrity = z3.load_cohort(round0_dir)
    ...
    zero = json.load(open(os.path.join(zero_dir, "zero_emission_v3_summary.json")))
...
def build_scenario(cell):
    ...
    static = np.full(ig.N_DC, ig.HOST_IDLE_W * ig.HOSTS_PER_SITE, dtype=float)

    acc = w3.accepted(z3.key_of(cell))
    if acc is None:
        raise RuntimeError(f"cell {cell['cell_id']} has no accepted load; "
                           f"the preflight should have stopped the run")
    wl = acc["workload"]
    budget = w3.budget_for(wl, cell["budget_fraction"])
    sc = Scenario(green_w=green, static_w=static,
                  brown_factor=list(ig.BROWN_FACTORS), green_factor=list(ig.GREEN_FACTORS),
                  cap_pes=[ig.CAP_PES_PER_SITE] * ig.N_DC,
                  arrival=wl["arrival"], runtime=wl["runtime"], pes=wl["pes"],
                  deadline=wl["deadline"], dyn_w_per_pe=ig.DYN_W_PER_PE,
                  per_job_wait_max=wl["wait_cap"], budget_total=budget)
    gres = np.maximum(green - static.reshape(-1, 1), 0.0)
    demand = p["concurrency"] * p["pes_per_job"] * ig.DYN_W_PER_PE
    prov = {"rho_residual": float(demand / max(float(gres.mean()), 1e-9)),
            "pes_share": float(p["pes_per_job"]) / ig.CAP_PES_PER_SITE,
            "is_fluid_control": p["pes_per_job"] in ig.FLUID_CONTROL_PES,
...
def phase_a(cells, out_dir, provenance):
    ...
    if not valid_everywhere:
        ...
        _write(os.path.join(out_dir, "round1_v3_blind_freeze.json"), art)
        return None, art
    art["status"] = "FROZEN"
...
def phase_b(cells, frozen, freeze_art, out_dir, provenance):
    ...
    _write(os.path.join(out_dir, "round1_v3_rows.jsonl"), rows, lines=True)
    _write(os.path.join(out_dir, "round1_v3_summary.json"), summary)
    return summary
...
def main(phase="a", round0_dir=None, zero_dir=None, out_dir=None):
    round0_dir = round0_dir or os.path.join(HERE, "round0_v3_out")
    zero_dir = zero_dir or os.path.join(HERE, "zero_emission_v3_out")
    out_dir = out_dir or os.path.join(HERE, "round1_v3_out")
    ...
    freeze_path = os.path.join(out_dir, "round1_v3_blind_freeze.json")
    if phase == "a":
