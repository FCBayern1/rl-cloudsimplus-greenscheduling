...
## H1a 诊断波收官(08-17 18:13,有效性门离线重算;正式判决权在 H1c)

oracle-informed 门(旧聚合规则,含双时域 bug=预算偏宽):
**有效 8 对(无效: offset 1009/245),改善 6/8,中位 −4.4%**,
逐对 [(0,−17.8%),(2018,+5.0%),(3027,−10.2%),(4036,−1.7%),(1254,−5.9%),
(2263,+4.1%),(3272,−13.4%),(4281,−2.9%)]。
FT 门:有效 9 对,改善 8/9,但中位仅 −0.6%(defer 每局仅 27 次,幅度≈零)。

**诊断读法**(不作判决):①方向存在:6/8 有效改善,且大幅改善集中在脏窗口
(0/3027/3272:immediate 碳 0.36/0.22/0.31 档),干净窗口反受损(2018/2263 为正)
——时移收益呈机会条件分布,与物理预期一致;②幅度:中位 −4.4% 低于 10% 判决线,
...
## 修订 A′(08-18 00:0x,Codex 拍板;全量六臂放行前预注册)

**冒烟门修订**(替代"每 offset 动作必分叉"):
1. persistence ≡ immediate 逐位;2. oracle/shuffle/anti 的原始瓦特 bins 必须不同;
3. ≥1 个固定冒烟 offset 上派生特征或 gate 动作分叉(ep0 已满足);
4. 0.9818 恒定指纹不得重现。
ep1 记为**策略不敏感窗口**,非接线失败(机制:近二值 gain + 阈值门 → 真话谎话在
"预算内存在免费 bin"上常给同一答案;仅整窗低谷才分叉)。

**全量统计规则(主结果)**:纳入所有过完成率合同的 offset;
**动作相同、碳相同的窗口按 0 效果计入,不得剔除**(按处理结果筛样本会人为放大
预测敏感度);报告整体中位数、胜负符号、有效配对数。

**诊断表(机制/条件性,不得替代总体价值)**:bins 可分率 → 派生特征可分率 →
gate 动作可分率 → 物理结果可分率的级联,以及可分窗口上的条件效果。

**暂不进入特征 B 方案**(避免追着敏感度设计特征);全量结果后分支:
clean>persistence 且 anti/shuffle 整体退化 → 门已证预测载重,不做 B;
oracle 有优势但污染臂大量相同 → 信息压缩过强,进 B;
oracle 自身无稳定优势 → 重做门。
B 若启动,特征采用 Codex 的逐作业棕电能积分定义
E_brown(h)=Σ_τ max(0, D_cf − Ĝ(h+τ))Δt(候选启动时刻覆盖整个运行期),
非简单窗口覆盖率。

**ep0 读数的解释边界**(Codex 提醒,预登记):oracle −8.2%/anti −8.9%/shuffle −7.4%
同向改善,尚不证明"真预报优于谎报"——可能是"适量等待平滑队列"的非预测收益;
该问题正由全量的 anti/shuffle 总体统计回答。


---

## TimeCAP 技能测量(08-18 01:2x,112 段,与 godeye 逐点对账;跑中不改任何配置)

| 指标 | 读数 |
|---|---|
| MAE 技能比 vs persistence | **1.97**(56.5W vs 28.7W,容量 211–596W) |
| 预测-真值相关(中位) | 0.507 |
| 低谷检测 | 命中 95% / **误报 74%**(靠"几乎永远喊低谷"刷命中,系统性低估未来绿电) |

解释了 clean 臂行为:低估未来 → best_future 恒差 → 门基本不开(6.5k vs oracle 284k
次 defer)。结构性原因:①**pred_len=144 视野截断**(门需 3600 步,超界 bins 夹在第
144 步值——clean 臂戴 4% 视野考试;pred_len 为模型属性,扩展需重训或自回归滚动);
②stretch-6 分布漂移;③blend 异常前科(memory 三次)与此一致。

**判决解释边界(预登记)**:clean 臂测的是"当前 TimeCAP 原样部署于本考场"的价值;
其失败不可外推为"任何现实预测器不可用"(视野截断是可修集成问题)。
clean-v2(修视野后)列为 H1c 之后的可选臂。


---

# ⭐ V3.2 战役终审(08-18,Codex 终审确认;正向认证路线冻结)

## 五条安全落档的结论

1. **Persistence 零点成立**,实验接线可信(逐位守零,10/10 offset);
2. 当前二值门下 oracle/shuffle/anti 总体效果几乎相同(各 −1.8% 中位,9/9)——
   **预报内容没有可认证的因果贡献**(级联:bins 20/20 可分 → 决策仅 4/20 可分 →
   可分处条件效果 −0.01%);
3. TimeCAP clean 臂未兑现收益(−0.2%),**结论仅限当前预测器、视野(pred_len=144)
   与集成方式**,不可外推为"现实预测器都不行";
4. 本场景最强已测试时间策略仅 1.8%(逐作业门)–4.4%(聚合门)中位,低于 10–13%
   认证噪声底——**不足以支持继续投入 PPO、蒸馏锚或新信用分配工程**;
5. **V3.2 正向认证路线冻结为干净负结果。**

## 换考场铁律(预注册,永久生效)

> 新场景先用【冻结强空间基线 + clairvoyant 时间启发式】做 headroom 预检;
> 只有中位改善 ≥10%、≥8/10 同向、完成率合格,才允许进入特征/BC/PPO 阶段。

新场景优先:跨 DC 绿电高度相关、同步枯竭、更长 slack(空间路由无法替代时间搬移)。

## 论文声称边界(Codex)

"空间替代性吃掉了预测价值"目前是**证据支持的解释,不是已证明的根因**。若论文要
作为根因声称,需补便宜归因实验:逐步减少可选 DC 数 / 提高绿电相关性,观察 oracle
temporal headroom 是否随空间自由度下降而上升。未做之前只能写解释性表述。

## 战役遗产(可复用资产)

判决器(有效性门/级联诊断/六臂协议)、分层解码修复、SLA 对齐(_p02)、
反事实需求特征(job_counterfactual_v1)、A′ 容量镜像扰动、TimeCAP 技能测量方法、
以及 R0/H0/H1a/H1c 的完整因果账本。
