...
### ⭐ 认证链三连 PASS + 纯 BC 物理短板(08-16 18:27)

| 环节 | 判决 |
|---|---|
| EXPLORE3(量纲修正后的真首次-defer 过滤,v1 数据) | 六条件门 **PASS**(delta +0.0789,双单调 100%,基线 0.8315,hold:route=19.9k:28.7k) |
| DATAGEN3(θ=0.6 + backlog_cap 200 + horizon_margin 600) | **配对门 24/24 PASS** |
| 认证 BC(v3 数据) | **PASS**(delta +0.1189,双单调 100%,基线 0.6451) |
| 纯 BC 物理评测(clean argmax 3 局) | **cc=0.504 @ 97.31%**——劣于 control(0.4045),不达合同 |

**判读**:①前两轮"卫生后仍饱和"结论作废——defer_count 归一化 bug 让过滤器空转,
真过滤后标签天然平衡;②方向问题就此关闭:gate 的 gain/slack 响应可由监督信号建立,
幅度真实(非饱和基线上 +0.12);③纯 BC 无奖励优化,物理不可用是预期内的模仿间隙,
不推翻 L 分支;④下一步按 §5 步骤 3:从认证 BC checkpoint **热启动 PPO 微调**
(真实奖励),逐 ck 探针监控 teacher-unlearning(gain 单调性从正转负即标记),
这是预注册要记录的核心现象。


### ⭐ V3.2B 步骤 3 首臂:热启动 PPO 微调,300k 内无 teacher-unlearning(08-16 19:47)

热启动经 `RLModuleSpec.load_state_path` 接线(V32B_WARM_START_GLOBAL,坏路径 fail-fast),
ck0 集成验证通过(ck2 delta +0.1261 ≈ BC 签名 +0.1189;随机初始化本底是 −0.01)。

| ck | delta | 基线 P(defer|not-worth) | gain/slack 单调 |
|---|---|---|---|
| ck2 | +0.1261 | 0.626 | 100%/100% |
| ck3 | +0.1098 | 0.688 | 100%/100% |
| ck4 | +0.0969 | 0.697 | 100%/100% |
| ck5(300k) | **+0.1114** | 0.673 | **100%/100%** |

**判读**:预注册最关心的"退火过程中 gain 单调性是否被洗掉"在纯热启动(imitation 系数
=0 的极限)下答案是**否**——300k 真实奖励训练全程保持六条件门水平的响应。
这与 R0 的"奖励偏好老师行为"闭环一致:PPO 不是想删掉这个映射,只是自己发现不了。
微调后策略 = 全战役第一个经 RL 训练仍带正确预报响应的策略。物理评测进行中。


### 600k 同预算正面对照(08-17 04:30,seed 1,clean argmax 3 offset)

| 臂 | 碳 | 完成率 | 探针 |
|---|---|---|---|
| FT(BC 热启动) | **0.4509** | **97.94%** | delta +0.0577(半衰),基线 0.819,双单调 100% |
| 盲臂 | 0.3814 | 88.72% | — |
| (参照)老师 | 0.268 | 100% | — |
| (参照)no-defer 启发式 | 0.343 | ~99.6% | — |

**判读**:①两臂都不达 99.5% 合同,严格 iso 比较不成立;②FT 完成纪律碾压盲臂
(+9.2pp),碳高 18%——盲臂的低碳是掉活折价(与其 300k 快照 0.367@88.9% 几乎没动,
说明盲臂在此配方卡死在掉活模式);③FT 300k→600k:碳 0.492→0.451 持续向好,但完成率
99.1→97.9 回落、探针响应半衰(+0.111→+0.058)——**PPO 在用响应锐度换碳,同时纪律
在松**。三条曲线一起看,正式退火 imitation(§5 既定配方)从"可选"升为"必需":
纯热启动保不住 600k 尺度的映射与纪律。
**下一步(晨会决策)**:退火 imitation loss 的实现方案两个候选——(a) BC 数据锚
(learner 混合离线批,经典,工程量 ~1 天);(b) 权重近端锚(L2 拉向 BC 快照,便宜但
偏离 §5 字面)。按纪律先写规格供复审,不夜间自行拍板。机器空档先补 seed 2 对
(既定协议复用,双种子证据反正要)。
