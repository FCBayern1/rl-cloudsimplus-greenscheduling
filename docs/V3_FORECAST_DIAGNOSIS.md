...
## 2d. ⭐ V3.1 参照波判决(08-15 10:07,600k × 4 臂,统一 drain,配对)

V3.1 = 全部公平性修复(no_offset + incremental_urgency + centered_zscore + fixed drain
+ obs_v31)但 **defer 头无预报直连边**。这是"间接通路"的天花板参照,也是 V3.2 的
文档化阴性对照。协议:600k、argmax、10 局、`--local drain`、同种子同 offset。

| seed | oracle | 完成率 | blind | 完成率 | oracle 相对 |
|---|---|---|---|---|---|
| s1 | 0.3651 | 98.27% | 0.2771 | 98.86% | **+31.8%(更差)** |
| s2 | 0.2671 | 98.52% | 0.2514 | 99.51% | **+6.2%(更差)** |

**两个种子一致:预报臂都更差,且完成率也不占优。**间接通路的天花板 = 负值。

⚠️ 读法边界:①iso 规则(≥99.5%)下只有 nofc_s2(99.51%)一格合格,严格的 iso 比较
做不了——但**配对内两臂完成率差 ≤0.99pp**,碳差 31.8%/6.2% 远大于完成率差能解释的量;
②s1 的 +31.8% 与 s2 的 +6.2% 相差很大,单看任一种子都不足为凭,一致的是**符号**;
③本波与旧 v3 表(0.22–0.28)数值不可直接比,配方不同(V3.1 有 centered_zscore 等)。

**对 V3.2 的意义**:V3.1/V3.2 只差 direct edge + 作业对齐特征 + 双尺度。参照波给出的
基线是"预报臂差 6–32%";V3.2 若能把符号翻正,那三项改动就是可归因的载重因素。

## 2e. ⭐ V3.2 Gate 2 判决 + 根因(08-15 11:25)

**Gate 2 = FAIL,且本次可判**(A5 判据满足:delta 与初始化明显不同,
−0.0125(未训)→ −0.0046/−0.0059/−0.0070;job 通道 TV 是惰性通道的 30–45×)。

| 判据 | 结果 |
|---|---|
| job_temporal_delta ≥ +0.05 | ❌ −0.0070 |
| P(defer) 对 forecast_gain 单调 | ❌ 0%(**方向相反**) |
| P(defer) 对 slack 单调 | ✅ **100%**(每个 ck 都对) |
| 判可性 | ✅ 30–45× 惰性通道 |

**关键线索:gate 把 slack 学对了、把 gain 学反了。**说明作业对齐通道是通的、
gate 能学,问题出在 gain 这一维的**奖励方向**。

### 根因:当前权重下"等到绿电"只在 ≤450 秒时才赚,而绿电峰周期是 1029 秒

中位作业(14.9M MI),锁定权重(w_c=0.5, w_urgency=2.0, W=3600, μ=3.524, σ=2.512, γ=0.999):

| 等待 | urgency 成本 | 碳收益(折现) | 净值 |
|---|---|---|---|
| 300s | −0.156 | +0.278 | **+0.122 赚** |
| 600s | −0.340 | +0.145 | −0.195 亏 |
| 1066s | −0.680 | +0.003 | **−0.677 亏** |
| 1800s | −1.352 | −0.121 | −1.473 亏 |

**盈亏平衡点 ≈450 秒;v3 的绿电峰周期 1029 秒。**真实场景需要的等待,在奖励上净亏。

### 为什么是 gain 这一维学反

离线统计(7170 个作业-时刻,6 个闭卷 offset):**corr(gain, time_to_best) = +0.235**,
且按 gain 四分位分组:

| gain 分位 | 中位等待时长 | 需等 >450s 的比例 |
|---|---|---|
| 低 25% | 922s | 60.3% |
| 25–50% | 1184s | 89.1% |
| 50–75% | 1318s | 92.1% |
...
**gain 越高的作业,100% 都需要等待超过盈亏平衡点。**所以在当前奖励下,
"高 gain → 少等" **是正确的策略**——gate 学的没错,是奖励错了。
探针里我把 gain 与 time_to_best 解耦(gain 高但 time_to_best=0.1),
gate 仍按训练中学到的 gain 权重给出负响应,与此一致。

### ⚠️ 真值表为什么没拦住(我的设计漏洞,留档)

真值表 row 2("棕电现在 / 绿电将至 / slack 充足 → defer 优于 route")用的是
**300 步**等待——恰好落在盈亏平衡点内侧,所以通过。**它测的等待时长比考场实际需要的
短 3 倍。**修正:真值表的等待时长必须由考场的绿电峰周期决定(此处 ≥1029s),
而不是任选一个短值。
...
4. **v3 的碳杠杆到底有多大**。上界问题:在 v3 的真实动力学里,
   ...
   **θ=0.5 更强:carbon 0.2864 @ 100%,Δ = −29.1%,green 49.2%**(更激进的等待
   仍然零完成率代价——slack-margin 逻辑稳健)。
   v3 的时间杠杆物理上存在且够大;七连 null 的责任**全部落回算法/信用分配侧**,
   考场无罪。论文可写的 oracle-gap:**完美用预报值 21–29%,RL 目前兑现 0%**
   —— V3.1/gate/蒸馏就是在追这 29 个点。蒸馏保底的老师即此策略(θ=0.5 版)。
   (仪器为 godeye 作弊型,29% 是可达下界而非理论上界;θ 更低可能还有余量。)
...
9. **⭐ P3 风险(08-14 新,V3.1 首批探针发现):z-score 把空间碳信号压缩了 ~70 倍。**
   旧 fixed 归一除 0.05,新 centered_zscore 除 σ=2.512——route-vs-defer 的门槛修对了,
   但 DC-vs-DC 的碳价差从 ±20 量级缩到 ±0.3 量级。300k 探针实测:control 通道
   TV=0.003(旧配方 600k 时 0.82),策略几乎不再区分"哪个 DC 现在绿"。
   若 600k 后空间选择仍死,碳收益可能兑现不了(P3 败因预埋)。
   候选修法(P3 失败才动,预注册纪律):w_carbon 0.5→2 补偿尺度、或空间项单独用
   scale_only、或逐决策候选均值中心化(§6b,它天然放大候选间差异)。
