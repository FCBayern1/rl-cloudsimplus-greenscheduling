...
> **修订记录 A2(08-14 12:20)**:A1 的前置条件(control TV ≥ 0.1)在新配方下
> **结构性不可达**——centered_zscore 把 DC 间碳价差压缩约 70 倍(旧 fixed 归一
> 0.05 vs σ=2.512),空间通道的学习信号 by design 远弱于旧配方,control 永远到不了
> 0.1 也不代表策略不成熟。改用**噪声底参照**:可判性 = forecast TV ≥ 10 × null TV
> (通道显著高于测量噪声)。一致性核验:该判据回溯应用于全部旧臂**不翻转任何历史
> 判决**(旧 oracle:forecast 0.38–0.49 ≫ null,判可,符号负;sp:0.007–0.017 vs
> null 0.001–0.002,边缘判可,符号≈0——与既有结论一致)。P1 判据本身(Δ>0 双种子)
> 自始未动。同时把"control 通道死寂=空间信号被压缩"记为 **P3 风险**(见诊断 §7.9)。
